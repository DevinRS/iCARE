...
def run_analysis(df: pd.DataFrame, y_col: str, number_of_features: int, static_features: list = None, iteration: int = 1, lw: bool = False) -> np.ndarray:
    global_prediction = []
    icare_prediction = []
    y_actual = []
    if lw:
        global_lw_prediction = []
        icare_lw_prediction = []
    df_original = df.copy()
    for i in range(iteration):
        ...
        icare_recommendation_list, weights = icare_recommendation(df_case, sample, y_col)
        ...
        # if lw:
        #     global_lw_prediction.append(model_global_lw.predict(global_sample.drop(y_col, axis=1))[0])
        #     icare_lw_prediction.append(model_icare_lw.predict(icare_sample.drop(y_col, axis=1))[0])

        global_prediction.append(model_global.predict_proba(global_sample.drop(y_col, axis=1))[:, 1][0])
        icare_prediction.append(model_icare.predict_proba(icare_sample.drop(y_col, axis=1))[:, 1][0])
        y_actual.append(sample[y_col].values[0])
        if lw:
            global_lw_prediction.append(model_global_lw.predict_proba(global_sample.drop(y_col, axis=1))[:, 1][0])
            icare_lw_prediction.append(model_icare_lw.predict_proba(icare_sample.drop(y_col, axis=1))[:, 1][0])

        if i%(iteration/10) == 0:
            print(print_statement)

    if lw:
        return np.array(global_prediction), np.array(icare_prediction), np.array(y_actual), np.array(global_lw_prediction), np.array(icare_lw_prediction)
