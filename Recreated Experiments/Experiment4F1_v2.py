from run_analysis import run_analysis_eguided_split
import pandas as pd
...
for size in starting_feature_size:
    ...
    # Run the analysis
    global_prediction, icare_prediction, eguided_prediction, y_actual = run_analysis_eguided_split(df, target, size, static_features=starting_features, iteration=20, split=0.2)
    ...
    for split_number in range(len(global_prediction)):
        ...
        roc = [global_roc, icare_roc, eguided_prediction_roc]
        ...
        global_accuracy = f1_score(y_actual[split_number], global_prediction[split_number], average='macro')
        icare_accuracy = f1_score(y_actual[split_number], icare_prediction[split_number], average='macro')
        eguided_accuracy = f1_score(y_actual[split_number], eguided_prediction[split_number], average='macro')

        accuracy = [global_accuracy, icare_accuracy, eguided_accuracy]
...
axes[0].errorbar(starting_feature_size, [x[2] for x in roc_result], label='eGuided', fmt='o-', yerr=[x[2] for x in roc_stderr])
...
axes[1].errorbar(starting_feature_size, [x[2] for x in accuracy_result], label='eGuided', fmt='o-', yerr=[x[2] for x in accuracy_stderr])
axes[1].set_xlabel('Number of Initial Features')
axes[1].set_ylabel('F1 Score')
axes[1].set_title('F1 Score for Different Methods')
axes[1].legend()

plt.tight_layout()  # Adjust layout for better spacing
plt.savefig('ExperimentResult/Experiment4_F1V2.png')
